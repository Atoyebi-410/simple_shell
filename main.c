 ...
 * when the signal SIGINT (control C) is send to the program
 ...
 * @data: This is the pointer to the struct of data
 * @argv: This is an array of arg passed to the program execution
 * @env: This is the environ passed to the program execution
 ...
 * @data: This is an infinite loop that shows the prompt
 */
...
void sisifo(char *prompt, data_of_program *data)
{
	...
	while (++(data->exec_counter))
	{
		...
		{
			free_all_data(data);
			exit(errno); /* if EOF is the first character of string, and exit*/
		}
		if (string_len >= 1)
		{
			expand_alias(data);
			...
			tokenize(data);
			if (data->tokens[0])
			{ /* this checks if a text is given to prompt, and execute */
